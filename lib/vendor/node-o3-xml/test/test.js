...
test37 : function() {
	return xml.parseFromString(readFile("test16.xml")).
    documentElement.prefix + " " + xml.parseFromString(readFile("test16.xml")).
...
for (var i in test) {
    ...
    if (test[i]())
        console.log(test[i]() + " test " + i + " succeeded\n");
    else {
        console.log(test[i]() + " test " + i + " failed\n");
        break;
    }
}
